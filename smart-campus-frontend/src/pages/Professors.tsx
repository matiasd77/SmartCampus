  ...
  const filterOptions = [
    {
      key: 'status',
      label: 'Status',
      options: PROFESSOR_STATUSES.map(status => ({ value: status.value, label: status.label })),
    },
    {
      key: 'rank',
      label: 'Rank',
      options: PROFESSOR_RANKS.map(rank => ({ value: rank.value, label: rank.label })),
    },
    {
      key: 'department',
      label: 'Department',
      ...
      <div className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          ...
          <SearchAndFilters
            ...
            onCreateClick={handleCreateProfessor}
            createButtonText="Add Professor"
          />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {professors.map((professor) => (
                  <ProfessorCard
                    key={professor.id}
                    professor={professor}
                    onEdit={isAdmin ? handleEditProfessor : undefined}
                    onDelete={isAdmin ? handleDeleteProfessor : undefined}
                    showActions={isAdmin}
                  />
                ))}
              </div>
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">
                {Object.keys(filters).length > 0
                  ...
                  : 'Get started by adding a new professor.'}
              </p>
              {isAdmin && (
                <div className="mt-6">
                  <button
